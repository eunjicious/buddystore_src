...
void PrimaryLogPG::do_op(OpRequestRef& op)
{
  ...
  dout(4) << "EUNJI T1 " << __func__ << " " << op->get_reqid() << dendl;
...
//      dout(4) << "EUNJI T4 " << __func__ << "[[" << op->get_reqid() << "]]" << dendl;
//      ldpp_dout(pg, 4) << " EUNJI T4 " << __func__ << m->get_reqid() << dendl;
...
void PrimaryLogPG::eval_repop(RepGather *repop)
{
  ...
  if (repop->all_committed) {
    dout(4) << "EUNJI T4 " << __func__ << " " << repop->op->get_reqid() << dendl;
