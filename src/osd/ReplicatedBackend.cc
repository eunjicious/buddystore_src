  ...
  OpRequestRef orig_op)
{
  ...
  // EUNJI
  dout(10) << "in_progress_ops insert tid = " << static_cast<uint64_t>(tid) << dendl;
  ...
  op.waiting_for_commit.insert(
   ...
   dout(4) << "EUNJI T5 " << __func__ << " " << reqid << dendl; 
  ...
  dout(4) << "EUNJI T2 " << __func__ << " " << orig_op->get_reqid() << dendl;
  ...
  InProgressOp *op)
{
  ...
  if (op->done()) {
    ...
    // EUNJI 
    dout(10) << "in_progress_ops erase tid = " << static_cast<uint64_t>(op->tid) << dendl;
...
void ReplicatedBackend::do_repop_reply(OpRequestRef op)
{
  ...
  dout(4) << "EUNJI T6 " << __func__ << " " << r->reqid << dendl; 
