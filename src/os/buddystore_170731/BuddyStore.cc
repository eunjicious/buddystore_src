...
{
// EUNJI
// 1. create global files: map (coll map, object map), checksum, super file 
// 2. initialize with null data 

// map: object_map, coll_map
// checksum: trans info, crc32c ==> crc32c 는 현재 bufferlist 에서 api 로 제공. 그런데 bufferlist 를 따로따로 file 에 쓰려고 하면.. 가능한가. bufferlist.crc32c 함수 확인할 것.  
// super: offset of root (in map file) 
  string fsid_str;
  int r = read_meta("fs_fsid", &fsid_str);
  if (r == -ENOENT) {
    uuid_d fsid;
...
#ifdef EUNJI
  buddyfs.create_meta_file();

#if 0
  dout(10) << " [EUNJI] " << "coll_t : "<< sizeof(coll_t) << "ghobject_t : " << sizeof(ghobject_t) << dendl; 
  dout(10) << " [EUNJI] " <<  __func__ << " create map and csum files " << dendl;

  int flags = O_RDWR | O_CREAT;
  int fd;

  // create a map file 
  fn = path + "/buddy_coll_map";
  fd = ::open(fn.c_str(), flags, 0644); 
  if (fd < 0) {
    dout(10) << " [EUNJI] " << __func__ << " failed to create map file" << dendl;
    return fd;
  }
  ::close(fd);
  ...
  fd = ::open(fn.c_str(), flags, 0644);
  if (fd < 0) {
     dout(10) << " [EUNJI] " << __func__ << " failed to create csum file" << dendl;
    return fd;
  }
  close(fd);
#endif
...
int BuddyStore::queue_transactions(Sequencer *osr,
...
void BuddyStore::_do_transaction(Transaction& t)
{

#ifdef EUNJI
  // create BuddyTransaction 
  BuddyTransaction bt;
#endif
...
//	::encode(*op, bt.log_bl);
	...
	uint32_t fadvise_flags = i.get_fadvise_flags();
        bufferlist bl;
        i.decode_bl(bl); // transaction 의 data_bl 에서 새로운 bl 로 copy 해주고 있음. 
#ifdef EUNJI
//	bt.op_encode(cid, oid, op->op);
	r = _write(cid, oid, off, len, bl, bt, fadvise_flags); 
#else
    ...
    case Transaction::OP_ZERO:
      {
        ...
        uint64_t len = op->len;
#ifdef EUNJI
	// zero 가 되는건 여기서 op 로깅하고- 실제 write 되면 거기서 bh 는 추가할 것임.
//	::encode(*op, bt.log_bl);
	r = _zero(cid, oid, off, len, bt);
#else
...
#ifdef EUNJI
//	::encode(*op, bt.log_bl);
	// garbage ? 만약 truncate 가 더 작게 되었따면 
#endif
...
#ifdef EUNJI
//	::encode(*op, bt.log_bl);
	// garbage ? remove 된 object 추가해야 함. 
#endif
	r = _remove(cid, oid);
      }
      break;
...
#ifdef EUNJI
//	::encode(*op, bt.log_bl);
	//r = _rmattr(cid, oid, name.c_str(), bt);
...
int BuddyStore::_write(const coll_t& cid, const ghobject_t& oid,
		     uint64_t ooff, size_t bytes, const bufferlist& bl,
		     BuddyTransaction& bt, uint32_t fadvise_flags)
{

  dout(10) << __func__ << " " << cid << " " << oid << " "
	   << ooff << "~" << bytes << dendl;
  assert(bytes == bl.length());
  ...
  // 1. get file space 
  // 2. set bl_head 
  // 3. add bl to bl_head's data_bl 
  // 4. return  

  int rv = 0; // number of added log entries 

  buddy_buffer_head_t* bh = new buddy_buffer_head_t();
  uint64_t foff;

  int r  = c->get_fspace(comp_t::data, oid, ooff, bl.length(), foff);

  if (r < 0 ){
    return -ENfo.OENT;
  }

  // append buffer_head info 
  bh->foff = ooff; 
  bh->fbytes = bl.length(); 
  bh->data_bl.append(bl);
  ...
  buddy_log_entry_t le;
  //* le = new buddy_log_entry_t(cid, oid, Transaction::OP_WRITE, arg_bl);
  //le->encode(bt.log_bl);
...
int BuddyStore::_zero(const coll_t& cid, const ghobject_t& oid,
		    uint64_t offset, size_t len, BuddyTransaction& bt)
{
  dout(10) << __func__ << " " << cid << " " << oid << " " << offset << "~"
	   << len << dendl;
  bufferlist bl;
  bl.append_zero(len); // 0 으로 된걸 length 만큼 추가하고 싶으면 이렇게 하면 되는구나. 
  return _write(cid, oid, offset, len, bl, bt);
}
#else
int BuddyStore::_zero(const coll_t& cid, const ghobject_t& oid,
...
//#ifdef EUNJI
  ...
  // 공간 할당하기 전에 lookup 해봐야 하는지도. 
  // 기존에 있던 공간에.. update 하거나 해야할듯. 
  int r  = c->get_fspace(comp_t::xattr, oid, bh->off, bl.length());

  if (r < 0 )
    return -ENOENT;

  // append buffer_head info 
  bh->len = len = bl.length(); 
  bh->bl.append(bl);

  bt.bh_list.push_back(*bh);

  // 실제 data 말고. meta log 에 남기기 위한 정보 필요함. 
  // object id 와 할당받은 data 의 위치 등. 
  // 이 함수 부르기 전에 encode 한건 op 자체이고 
  ::encode(*bh, bt.log_bl); 
  rv ++;


  // 사실 in-memory data structure 업데이트 하는 부분도 다 추가해야 함. 
  // 기존의 것들은 storage 데이터는 신경 안쓰고 있으니께. 
//#endif
#endif
...
//#ifdef EUNJI 
//int BuddyStore::get_cfname(const coll_t& cid, char *s, int len)
//{
//  const string &cid_str(cid.to_str());
//  return snprintf(s, len, path.c_str(), cid_str.c_str());
////return snprintf(s, len, basedir.c_str(), cid_str.c_str());
//}
//#endif
//
int BuddyStore::_create_collection(const coll_t& cid, int bits)
{
  ...
  // 해당 file 에 있는지 없는지를 먼저 찾아야 함. 
  flags |= O_CREAT;

//  get_cfname(cid, fn, sizeof(fn));
  fn = path + cid.to_str();
  dout(10) << __func__ << " " << "coll_fn : " << fn << dendl;
  ...
  // 2. store null data 
  // 3. Initialize position in data file in collection 
  result.first->second->set_pos(0);
  result.first->second->fn = fn;
  // 4. bitmap for each collection 
  // fix it. later. 
