...
//#ifdef EUNJI
//class BuddyStore : public ObjectStore, public BuddyFileStore{
//#else
...
private:
  ...
  int _zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len,
	    BuddyTransaction& bt); // bt
  int _truncate(const coll_t& cid, const ghobject_t& oid, uint64_t size);
  int _remove(const coll_t& cid, const ghobject_t& oid);
  //int _setattrs(const coll_t& cid, const ghobject_t& oid, map<string,bufferptr>& aset, BuddyTransaction& bt); // bt
  ...
  // collection operations: sync immediate  
...
//#ifdef EUNJI
//  int get_cfname(const coll_t& cid, char* s, int len); 
//#endif
