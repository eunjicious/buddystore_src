...
//    uint64_t o_object;
    uint64_t o_data; 
    uint64_t o_xattr;
    uint64_t o_omap_header;
    uint64_t o_omap; // offset within a data file 
    uint64_t o_bitmap; // offset within a bitmap file 
    ...
    s_offsetv_t(uint64_t d, uint64_t x, uint64_t h, uint64_t m) { 
      o_data = d;
      o_xattr = x;
      o_omap_header = h;
      o_bitmap = m;
  ...
  struct Collection : public CollectionImpl {
    ...
    bool exists;
#ifdef EUNJI
    map<string, s_offsetv_t> s_object_map;
#endif
...
#ifdef EUNJI
//---- buddy store data structures 
  int _meta_fd;
  int _coll_fd;
  int _data_fd;
  int _bitmap_fd;

  map<string, s_offsetv_t> s_coll_map; // <coll_id, offset> to be stored in meta file.  
#endif
...
#ifdef EUNJI
/////////////// Buddy Store operations ///////////
  int _buddy_load_meta_file(); // create metadata file (superblock)
//  int _buddy_load_object_file(); // create object file (object, collection)  
#endif

#ifdef EUNJI
  int _buddy_save_meta_file(); // update metadata file (superblock)
//  int _buddy_save_object_file(){}; // update object file (object, collection)  
#endif
