...
int BuddyStore::mount()
{
  int r;
#ifdef EUNJI
  r = _buddy_load_meta_file();

//  _buddy_load_bitmap_file();
//  _buddy_load_collection_file();
//  _buddy_open_data_file();

//  _buddy_load_object_file();
#endif
...
#ifdef EUNJI
    dout(10) << "[EUNJI] " << __func__ << "op = " << optable[op->op] <<" "<< op->op << dendl;
#endif
    switch (op->op) {
...
#ifdef EUNJI
// -----------------
// buddy store operaitons 

int BuddyStore::_buddy_load_meta_file()
{

  /// meta file 
  string fn = path + "/buddy.meta";
  int flags = O_RDWR;
  int r = ::open(fn.c_str(), flags, 0644);

  if (r < 0) {
    // dout(1) >> __func__ << "Failed to open buddy_meta_file" << dendl;
    dout(10) << "[EUNJI] " << __func__ << ": No meta file" << dendl;
    return r;
  }
  ...
  if(st.st_size == 0){
    dout(10) << "[EUNJI] " << __func__ << ": Empty meta file" << dendl; 
    return -1;
  }

  // load data 
  mbl.read_fd(_meta_fd, st.st_size);

  bufferlist::iterator p = mbl.begin();
  ::decode(s_coll_map, p);


  // dump s_coll_map 
  for(map<string, uint64_t>::iterator p = s_coll_map.begin();
      p != s_coll_map.end();
      p++) {
      dout(10) << "[EUNJI] " << __func__ << ": coll_idx = " << p->first << " : " << p->second << dendl;
  } 
...
#ifdef EUNJI
//----- save all collections in a meta file 

int BuddyStore::_buddy_save_meta_file()
{
  string fn = path + "/buddy.meta";

  // We will fix it to direct mode. 
  ...
  if (r < 0) {
    // error 
    dout(1) << "[EUNJI] " << __func__ << "Failed to open buddy_meta_file" << dendl;
    return r;
  ...
  for (ceph::unordered_map<coll_t, CollectionRef>::iterator p = coll_map.begin();
      ...
      p++)
  {
    cname = stringify(p->first); // cid
    s_coll_map[cname] = offset << 12; // cid
    offset++;

    dout(10) << "[EUNJI] " << __func__ << ": coll_idx = " <<  cname << " : " << s_coll_map[cname] << dendl;
  }

  // succeed in file access 
  _meta_fd = r; 

  bufferlist mbl;
  ::encode(s_coll_map, mbl);

  mbl.write_fd(_meta_fd); // what is the return value?
