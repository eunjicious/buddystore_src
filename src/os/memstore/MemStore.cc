...
#ifdef EUNJI
void MemStore::stat_object(const coll_t& cid, const ghobject_t& oid)
{
  ...
  dout(10) << "xattr count = " << o->xattr.size() << dendl;
  dout(10) << "omap_header length = " << o->omap_header.length() << dendl;

  uint64_t omap_size = 0;

  for(map<string,bufferlist>::iterator p = o->omap.begin(); 
     p != o->omap.end(); ++p) { 
    omap_size += p->first.size();
    omap_size += p->second.length();
  }
  dout(10) << "omap size = " << omap_size << dendl;
...
int MemStore::_load()
{
  ...
  dout(7) << "EUNJI: collections_info_fname = "<< fn << dendl;
  ...
  for (set<coll_t>::iterator q = collections.begin();
    ...
    dout(7) << "EUNJI: collection_fname = "<< fn << dendl;
    bufferlist cbl;
		      ...
		      const char *name, bufferptr& value)
{
  ...
  // EUNJI
  dout(10) << __func__ << " " << c->cid << " " << oid << " " << name << " ~" << value.length() << dendl;
  return 0;
}
		       ...
		       map<string,bufferptr>& aset)
{
  ...
  // EUNJI
  dout(10) << __func__ << " " << c->cid << " " << oid << " ~" << aset.size() << dendl;
    ...
    )
{
  ...
  *header = o->omap_header;
  ...
  // EUNJI
  dout(10) << __func__ << " " << cid << " " << oid << " ~" << header->length() << dendl;
  return 0;
}
...
//  dout(10) << __func__ << " " << cid << " " << oid << dendl;
  ...
  dout(10) << __func__ << " " << cid << " " << oid << " ~" << out->size() << dendl;
  return 0;
}
...
void MemStore::_do_transaction(Transaction& t)
{
  ...
  dout(10) << __func__ << " " << t.get_osr() << dendl;
			...
			map<string,bufferptr>& aset)
{
  ...
  // EUNJI
  dout(10) << __func__ << " " << cid << " " << oid << " ~" << len << dendl;
...
int MemStore::_omap_setkeys(const coll_t& cid, const ghobject_t &oid,
			    bufferlist& aset_bl)
{
  dout(10) << __func__ << " " << cid << " " << oid << " ~" << aset_bl.length() << dendl;
  //dout(10) << __func__ << " " << cid << " " << oid << dendl;
  ...
  while (num--) {
    ...
    ::decode(o->omap[key], p);
// EUNJI
    dout(7) << __func__ << " key = " << key << " value = " << o->omap[key] << dendl;
  }
...
int MemStore::_omap_setheader(const coll_t& cid, const ghobject_t &oid,
			      const bufferlist &bl)
{
  dout(10) << __func__ << " " << cid << " " << oid << " ~" << bl.length() << dendl;
  //dout(10) << __func__ << " " << cid << " " << oid << dendl;
  CollectionRef c = get_collection(cid);
  if (!c)
    return -ENOENT;
