    ...
    uint64_t& foff, size_t& alloc_bytes)
{
    ...
    // search and delete if overlapped extent 
    int r = delete_object_index(type, oid, ooff, bytes);
    if (r > 0) 
      ldout(cct, 10) << "deleted" << dendl;
...
// hash_to_bytes 정의 필요
int BuddyFileStore::BuddyCollFileObject::search_fiemap_hash(int type, const ghobject_t& oid, 
    const size_t bytes) 
{
  uint64_t bnum = (uint64_t)_reverse_bits(oid.hobj.get_hash());
  uint64_t foff = bnum << BUDDY_FILE_BSIZE_BITS; // hash(blk num) to bytes 
  ldout(cct, 10) << __func__ << " hash = " << bnum << " foff = " << foff << dendl; 
  return search_fiemap(type, oid, foff, bytes);
}



int BuddyFileStore::BuddyCollFileObject::search_fiemap(int type, const ghobject_t& oid, 
    const uint64_t foff, const size_t bytes)
{ 
  int collision = 0;

  uint64_t curr_foff = foff;
  size_t curr_bytes = bytes;
  uint64_t os, oe, ns, ne;

      
  for (map<uint64_t, buddy_oindex_t>::iterator p = buddy_fiemap.begin(); 
	    p != buddy_fiemap.end(); p++)
  {
    ...
    os = p->second.foff;
    oe = p->second.foff + p->second.alloc_bytes - 1;
    ns = curr_foff;
    ne = curr_foff + curr_bytes - 1;
    ldout(cct, 10) << __func__ << os <<"~"<< oe << " " << ns << "~" << ne << dendl;

    // 1. include 
	    
    if (ns >= os && ne <= oe){
      collision = -1;
      ldout(cct, 10) << "collision : include" << dendl;
      break;
    } 
    // 2. head_overlap 
    else if (ne >= os && ne <= oe){
      collision = -2;
      ldout(cct, 10) << "collision : head_overlap" << dendl;
    ...
    // 3. tail_overlap 
      ...
      collision = -3;
      ldout(cct, 10) << "collision : tail_overlap" << dendl;
...
int BuddyFileStore::BuddyCollFileObject::alloc_space_hash(int type, const ghobject_t& oid, 
    const uint64_t bytes, uint64_t& alloc_foff, uint64_t& alloc_bytes)
{

  // find ghoject at the hash location 
  uint64_t bnum = (uint64_t)_reverse_bits(oid.hobj.get_hash());
  uint64_t foff = bnum << BUDDY_FILE_BSIZE_BITS; // hash(blk num) to bytes 
  ssize_t limit_bytes = max_fbytes;

  //uint64_t loff = foff + bytes;
  buddy_oindex_t* nhindex;


  // 1. check its extent 
  map<uint64_t, buddy_oindex_t>::iterator p = buddy_fiemap.find(foff);

  if (p != buddy_fiemap.end()) { // found 

    if (p->second.oid != oid){ // extension 
      ldout(cct, 10) << "Collision " << dendl;
      return -1;
    }
    ...
    // check next node 
    map<uint64_t, buddy_buddy_oindex_t>::iterator np = p + 1;
      
    if (np != buddy_fiemap.end()) 
      limit_bytes = np->second.foff;

  }else { // search needed
    int r =  search_fiemap(type, oid, foff, bytes); 
    if (r < 0)
      return r;

    // new alloc 
    nhindex = new buddy_oindex_t(); 
  }

  vector<buddy_index_t>>::iterator tp = nhindex->components.begin(); 

  if (p != nhindex->components.end()){
     = tp->second.find(type);


  }

  for(vector<buddy_index_t>::iterator cp = 
      nhindex->components.begin(); 
      cp != nhindex->components.end(); cp++)
  {
    if (cp->type == 
