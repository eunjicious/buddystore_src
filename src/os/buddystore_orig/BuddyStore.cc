...
// EUNJI
  dout(7) << "EUNJI: bl.length = "<< bl.length() << dendl;
  dout(7) << "EUNJI: why? " << dendl;
...
int BuddyStore::_fopen(string& fn, int flags, int mode)
{
...
int BuddyStore::_buddy_load_meta_file()
{
  ...
  if (r < 0) {
    ...
    dout(10) << __func__ << "No meta file" << dendl;
    return r;
  }
  ...
  if(st.st_size == 0){
    dout(10) << __func__ << "Empty meta file" << dendl; 
    return -1;
  }

  // load data 
  cbl.read_fd(_mfd, st.st_size);
...
int BuddyStore::_buddy_save_meta_file()
{
  ...
  for (i = 0; i < 3; i++) {
    coll_id.append(to_string(i)); 
    coll_index[coll_id] = i * 4096;
    dout(7) << "EUNJI: coll_index" <<  coll_id << " : " << i*4096 << dendl;
  }
