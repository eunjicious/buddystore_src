    ...
    )
{
  ...
  *header = o->omap_header;
  ...
  int r = object_kvmap->get_header(oid, &kv_header);
  if (r < 0 && r != -ENOENT) {
    ...
    )
{
  ...
  set<string> kv_keys;
  int r = object_kvmap->get_keys(oid, &kv_keys);
  if (r < 0 && r != -ENOENT) {
    dout(10) << __func__ << " Failure in object_kvmap" << r << dendl;
    assert(0);
  }
    ...
    )
{
  ...
  int r = object_kvmap->get_values(oid, keys, &kv_out);
  assert(kv_out.size() == (*out).size());

  dout(10) << __func__ << " kv_out.size " << kv_out.size() << " r " << r << dendl; 
    ...
    )
{
  ...
  set<string> kv_out;
  int r = object_kvmap->check_keys(oid, keys, &kv_out);
  if (r < 0 && r != -ENOENT) {
    dout(10) << __func__ << " Failure in object_kvmap" << r << dendl;
    assert(0);
  }
							 ...
							 const ghobject_t& oid)
{
  ...
  // kvmap 
  //ObjectMap::ObjectMapIterator p = object_kvmap->get_iterator(oid);
  // return p;
			    ...
			    bufferlist& aset_bl, const SequencerPosition &spos)
{
  ...
  while (num--) {
    ...
    dout(10) << __func__ << " object_kvmap : key " << key << " value " << o->omap[key] << dendl;
  }
