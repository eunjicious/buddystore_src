...
#ifdef EUNJI
class BuddyStore : public ObjectStore, public BuddyFileManager{
#else
...
public:
  ...
  struct Collection : public CollectionImpl {
    ...
    map<string,bufferptr> xattr;
#ifdef EUNJI
    map<ghobject_t, vector<map_info>> bd_object_map;
#endif
...
#ifdef EUNJI
  map<coll_t, vector<map_info>> bd_coll_map; // sorted map - store coll_t instead of cid only.    
#endif
...
#ifdef EUNJI
  int _bd_touch(const coll_t& cid, const ghobject_t& oid);
  int _bd_write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len,
	      const bufferlist& bl, uint32_t fadvise_flags = 0);
  int _bd_zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len);
  int _bd_truncate(const coll_t& cid, const ghobject_t& oid, uint64_t size);
  int _bd_remove(const coll_t& cid, const ghobject_t& oid);
  int _bd_setattrs(const coll_t& cid, const ghobject_t& oid, map<string,bufferptr>& aset);
  int _bd_rmattr(const coll_t& cid, const ghobject_t& oid, const char *name);
  int _bd_rmattrs(const coll_t& cid, const ghobject_t& oid);
  int _bd_clone(const coll_t& cid, const ghobject_t& oldoid, const ghobject_t& newoid);
  int _bd_clone_range(const coll_t& cid, const ghobject_t& oldoid,
		   const ghobject_t& newoid,
		   uint64_t srcoff, uint64_t len, uint64_t dstoff);
  int _bd_omap_clear(const coll_t& cid, const ghobject_t &oid);
  int _bd_omap_setkeys(const coll_t& cid, const ghobject_t &oid, bufferlist& aset_bl);
  int _bd_omap_rmkeys(const coll_t& cid, const ghobject_t &oid, bufferlist& keys_bl);
  int _bd_omap_rmkeyrange(const coll_t& cid, const ghobject_t &oid,
		       const string& first, const string& last);
  int _bd_omap_setheader(const coll_t& cid, const ghobject_t &oid, const bufferlist &bl);

  int _bd_collection_hint_expected_num_objs(const coll_t& cid, uint32_t pg_num,
      uint64_t num_objs) const { return 0; }
  int _bd_create_collection(const coll_t& c, int bits);
  int _bd_destroy_collection(const coll_t& c);
  int _bd_collection_add(const coll_t& cid, const coll_t& ocid, const ghobject_t& oid);
  int _bd_collection_move_rename(const coll_t& oldcid, const ghobject_t& oldoid,
			      coll_t cid, const ghobject_t& o);
  int _bd_split_collection(const coll_t& cid, uint32_t bits, uint32_t rem, coll_t dest);
#endif
