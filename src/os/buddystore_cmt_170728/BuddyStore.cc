...
void BuddyStore::_do_transaction(Transaction& t)
{
  ...
  while (i.have_op()) {
	...
	r = _write(cid, oid, off, len, bl, fadvise_flags); // 그리고 그 bl 을 write 에게 넘겨줌. 
	// buddy store 에서는.. "transaction" 전체를 atomic 하게 처리해야 하니까 
	// 이걸 하나의 bufferlist 로 담아두고. file offset 을 각각부여해서 쓰도록 해야할 것 같음. 
      }
      break;
