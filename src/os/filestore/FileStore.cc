...
int FileStore::mkfs()
{
  ...
  dout(10) << "omap_backend " << superblock.omap_backend << dendl;
...
//EUNJI
  if(superblock.omap_backend == "memdb")
    snprintf(omap_fsid_fn, sizeof(omap_fsid_fn), "/var/lib/ceph/osd_uuid");
...
int FileStore::mount()
{
  ...
  if (!(generic_flags & SKIP_MOUNT_OMAP)) {
    ...
    dout(0) << "mount: skip_mount_omap is set" << dendl;
  }
...
int FileStore::umount()
{
  ...
  flush();
  sync();
  do_force_sync(); // 강제로 일하라고 깨우는 거고 

  lock.Lock();
  stop = true; // 이렇게 해놓고 
  sync_cond.Signal(); // 꺠우면 일어나서 stop 한거 보고 
  lock.Unlock();
  sync_thread.join();
  if (!m_disable_wbthrottle){
    wbthrottle.stop();
...
void FileStore::sync_entry()
{
  ...
  while (!stop) {
    ...
    if (!force_sync) { // 잠들기 전에도 한번더 체크. 
      dout(20) << "sync_entry waiting for max_interval " << max_interval << dendl;
      sync_cond.WaitInterval(lock, max_interval); // 여기서 잠들면 lock 을 놓음. 
    } else {
      dout(20) << "sync_entry not waiting, force_sync set" << dendl;
    }
    // 깨어났는데 force_sync 면 false 로 바꾸고 작업수행.  
    if (force_sync) {
