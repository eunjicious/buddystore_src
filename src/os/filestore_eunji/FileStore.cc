...
{
  // 이 함수는 실제 filesystem 에 반영하는 작업을 수행함. 
  //
  ...
  // sequencer 큐에 넣어둠. 
  // op 가 처음 실행될 Sequencer 라는게 만들어짐. 
  // Sequencer 는 현재 같은 Sequencer 에 묶인 애들이 진행되는 상태를 트래킹 하고 
  // 다 되면 그에 맞는 작업을 처리해주도록 되어 있음. 
  // 예를 들어 writeahead 인 경우 journaling 을 하기 전에 jq에 넣어둠. 
  // 그러고 나서 journaling 다 끝나면 jq 에서 꺼내서 q 에 넣어줌. 
  // 즉, 이 queue_op 함수를 부르는 것임. 
  // 그러면 실제 op 다 처리될 때 까지 기다림.. 
  // BuddyStore 도 동일한데 thread pool 이 처리하는 함수인 _do_op 에서 
  // 스토리지로 I/O를 보내지 않고 in-memory 만 업데이트 하면 됨. 
  ...
  // FileStore가 생성될 때 OpWq 를 만드는데 여기 queue 에 다가 넣어주면 
  // 이 큐하고 연계된 thread 들이 꺼내서 작업함. 
  op_wq.queue(osr);
}
...
// FileStore 는 트랜잭션을 통째로 저널링 해버리고 
// 현재 _do_op 에서는 while 문 돌면서 do_transactions 를 수행함. 
// buddystore 는 
...
// 원래 osd 레벨에서 셋팅한 ondisk 를 그냥 넘겨야 하는데 
// journalahead 를 한 경우에는 commit 끝나면 queueop 해줘야 하니까 
// wrapping 함수를 만들고 ==> _journaled_ahead 임 
// 그걸 호출함. 
// 그러면 그 함수에서 queueop 랑 osr 처리 해주고 다음단계로 넘어감.  
...
void FileStore::_journaled_ahead(OpSequencer *osr, Op *o, Context *ondisk)
{
  ...
  if (o->osd_op)
	dout(4) << "EUNJI T3 " << __func__ << " " << (static_cast<OpRequest *>(o->osd_op.get()))->get_reqid() << dendl;
  else	
	dout(4) << "EUNJI T3 " << __func__ << " " << o->osd_op << dendl; 
  ...
  // Finisher 는 그냥 queue 에 Context 넣어주면 
  // 자기가 알아서 꺼내서 처리함. 
  // buddystore 구현할 때 알아야 할 것은 collect_context 에서 받아온 애들을 
  // 어느 시점에 finisher 에게 보낼 것인가임. 
  // on_commit : logging 이랑 data write 끝나고. 
  // on_apply : in-memory 끝나고. 
  // on_apply_sync : in-memory 끝나고. on_apply 랑 같이? 
...
void FileStore::sync_entry()
{
  ...
  again:
	...
	new SyncEntryTimeout(cct, m_filestore_commit_timeout);
      ...
      dout(4) << "EUNJI T7 " << "sync_start " << start << dendl; 
	...
	if (err < 0) {
      ...
      dout(4) << "EUNJI T8 " << "sync_end " << lat << dendl; 
