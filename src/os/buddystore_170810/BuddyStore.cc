				 ...
				 ThreadPool::TPHandle *handle)
{
  ...
  // 이 부분은 do_transactions()로 묶어 버리고, 실제 logging 이나 hash file 에 써야 하는 것도 transactions 단위로 처리하도록 해버리자. 
  // 일단 do_transactions 를 한바퀴 돌아야 log 를 만들수 있을 것 같은데- 
  // transaction 주면 op 하고 argument 만 로깅하는 함수를 하나 짤까보네. 
  // 그래서 그걸 제일 먼저하고.. 근데 공간도 할당받아야 함.. 
  // 그래야 로그로 쓸 수 있음.. 
  // 이거 다 하고 나서 사실 in-memory update 해야하니까.- 
  // 현재있는 _do_transaction 은 사실 나중에 하는게 맞구나..  
  // 지금 buddyFileStore 에 구현해다 놓은 코드를 모아서 
  // 함수 하나로 만들고 아예 던져버리자. 
  // 그럼.. log_bl 이랑 iovector 로 구성된 bl 받아오기. 
  // log_bl 은 prepare_entry 하고 submit 해서 넘겨버리고 
  // iovector 는 돌면서 write 하기.
  // 이게 다 되어야만.. return 할 수 있으니까- 
  // 최적화 되어야 하는데.. 
  // log_bl 에 쓰는거랑 iov 쓰는거는 다 parallel 하게 하면 됨. 
  // fdatasync 를 각 파일에 해야하는데 이것도 순서는 상관없고.. 
  // direct io 로 해도 되고. journal 은 direct io 로 하고 iov 는 buffered 로 해도 되고.. 
  // 이게 다 끝나야지 겨우 readable 해 지는 것임. 
  // journal 에만 다 쓰면 되는게 아니고.. 
  // queue_o 를 만들어서 direct io 를 parallel 하게 쓰게 해야함.. 
  // 아 골치아파.. 
  //
...
#ifdef EUNJI_V1
//  BuddyTransaction bt;
  BuddyFileStore::BuddyTransaction* bt = bdfs.buddy_get_transaction();
#endif
  ...
  while (i.have_op()) {
#ifdef EUNJI_V1 // DEBUG
    bdfs.stat_buddy_coll_map();
	...
	bufferlist bdfs_bl;
        i.decode_bl(bdfs_bl); 
	r = bdfs.buddy_write(cid, oid, off, len, bdfs_bl, *bt, fadvise_flags); 
#endif
...
#ifdef EUNJI_V1
	r = bdfs.buddy_truncate(cid, oid, off, *bt);
#endif
      }
      break;
	...
	r = _remove(cid, oid);
#ifdef EUNJI_V1
	r = bdfs.buddy_remove(cid, oid, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_setattrs(cid, oid, to_set, *bt);
#endif
...
#ifdef EUNJI_V1
	r = bdfs.buddy_setattrs(cid, oid, aset, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_rmattr(cid, oid, name.c_str(), *bt);
#endif
      }
      break;
	...
	r = _rmattrs(cid, oid);
#ifdef EUNJI_V1
	r = bdfs.buddy_rmattrs(cid, oid, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_clone(cid, oid, noid, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_clone_range(cid, oid, noid, off, len, off, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_clone_range(cid, oid, noid, srcoff, len, dstoff, *bt);
#endif
...
#ifdef EUNJI_V1
	r = bdfs.buddy_create_collection(cid, op->split_bits, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_omap_clear(cid, oid, *bt);
#endif
      }
      break;
...
#ifdef EUNJI_V1
	r = bdfs.buddy_omap_setkeys(cid, oid, aset_bl, *bt);
#endif
      }
      break;
  ...
  _op_journal_transactions(bt.log_bl,); 
  // _do_op 로 정의해서 parallel 하게 처리. 
  //
#endif
